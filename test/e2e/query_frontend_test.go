...
func TestRangeQueryShardingWithRandomData(t *testing.T) {
	...
	for k, labels := range timeSeries {
		var timeStepSize = (startTime - endTime) / int64(30+1)
		...
		samplespb = append(samplespb, prompb.TimeSeries{
			...
			Samples: []prompb.Sample{
				{
					Value:     float64(k),
					Timestamp: currTimeStep,
				},
			},
		})
		currTimeStep += timeStepSize
	}
...
func TestRangeQueryDynamicHorizontalSharding(t *testing.T) {
	...
	// + rest (of interval) - Doubt - what is rest of interval?
